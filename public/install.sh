...
detect_all_drives() {
    ...
    case "$OS_TYPE" in
        linux)
            info "Scanning physical drives and partitions..."
            
            # Get all mounted block devices (actual drives/partitions)
            while IFS= read -r line; do
                local device=$(echo "$line" | awk '{print $1}')
                local mountpoint=$(echo "$line" | awk '{print $2}')
                local fstype=$(echo "$line" | awk '{print $3}')
                
                # Skip if not a real block device
                if [[ ! "$device" =~ ^/dev/(sd|hd|nvme|mmcblk|vd) ]]; then
                    continue
                fi
                ...
                # Skip root filesystem (Docker limitation)
                if [[ "$mountpoint" == "/" ]]; then
                    info "Skipping root partition (Docker limitation)"
                    continue
                fi
                
                # Skip boot partitions (usually too small and not needed)
                if [[ "$mountpoint" =~ ^/boot ]]; then
                    info "Skipping boot partition: $mountpoint"
                    continue
                fi
